...
export default function AdventureBooks(){
    
    const[AdvenBooks , setAdvenBooks] = useState([]);
    ...
    const FetchBooks = async ()=> {
        ...
        const response = await axios.get(
            `https://www.googleapis.com/books/v1/volumes?q=adventure+books&startIndex=${startIndex}&maxResults=${maxResults}`
        );

        const data = response.data;
        setAdvenBooks(data.items || []);
     }
     catch(error){
        setErrorMsg(true)
     }
      ...
      <h2 className="mb-4 fw-bold" style={{ color: "#3E2F5B"}}>Adventure Books</h2>
      <div className="row">
        {AdvenBooks.map((book, index) => {
